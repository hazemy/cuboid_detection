...
def draw_multi_lr():
    ...
    plt.legend(['lr $1e-5$', 'lr $1e-4$', 'lr $1e-3$', 'lr $1e-2$'], loc='upper right')
...
def draw_multi_lr_ap50_bbox():
    ...
    plt.legend(['lr $1e-5$', 'lr $1e-4$', 'lr $1e-3$', 'lr $1e-2$'], loc='lower right')
    plt.xlabel('Iterations')
    # plt.ylabel('Accuracy')
    fig_1.tight_layout()
    plt.grid(b=True, linestyle=':')
...
def draw_multi_lr_ap50_key():
    ...
    plt.legend(['key-lr $1e-5$', 'key-lr $1e-4$', 'key-lr $1e-3$', 'key-lr $1e-2$'], loc='center right')
    plt.xlabel('Iterations')
    # plt.ylabel('Accuracy')
    fig_1.tight_layout()
    plt.grid(b=True, linestyle=':')
...
def draw_train_val_best_lr():
    ...
    plt.legend(['training loss (lr $1e-3$)', 'validation loss (lr $1e-3$)', 'training loss (lr $1e-4$)', 'validation loss (lr $1e-4$)'], 
               loc='upper right')
    plt.xlabel('Iterations')
...
def draw_train_val_warmup():
    experiment_folder = '/home/porthos/masters_thesis/cuboid_detection/results/final_dataset_results/final'
    metrics_1 = load_json_arr(experiment_folder + '/21_10_20-2_00[no_warmup-lr0.001]' + '/metrics.json')
    metrics_2 = load_json_arr(experiment_folder + '/20_10_20-18_30[lr0.001]' + '/metrics.json')

    colors = ['#1F77B4', '#FF7F0E', '#2CA02C', '#D62728', '#8C564B', '#7F7F7F', '#9467BD', '#17BECF'] #blue, orange, green, red, brown, grey, purple, cian
    fig_1 = plt.figure()
    plt.plot(
        [x['iteration'] for x in metrics_1 if x['iteration']<=3000], 
        [x['total_loss'] for x in metrics_1 if x['iteration']<=3000], color=colors[4], label='no warmup')
    plt.plot(
        [x['iteration'] for x in metrics_2 if x['iteration']<=3000], 
        [x['total_loss'] for x in metrics_2 if x['iteration']<=3000], color=colors[7], label='with warmup')
    ...
    # plt.title('Total train-val losses at 0.001 and 0.0001 lr')
    plt.grid(b=True, linestyle=':')
    # plt.savefig('/home/porthos/masters_thesis/writing/figures/results/train_loss_warmup_comp.pdf')
    plt.show()
 
def draw_ap50_warmup():
    experiment_folder = '/home/porthos/masters_thesis/cuboid_detection/results/final_dataset_results/final'
    metrics_1 = load_json_arr(experiment_folder + '/21_10_20-2_00[no_warmup-lr0.001]' + '/metrics.json')
    metrics_2 = load_json_arr(experiment_folder + '/20_10_20-18_30[lr0.001]' + '/metrics.json')
    ...
    plt.plot(
        ...
        [x['bbox/AP50'] for x in metrics_1 if ((x['iteration']%300==299) and (x['iteration']<=3000))], label='$AP_{50}$ no warmup', color=colors[4], linestyle='-')
    plt.plot(
        [x['iteration'] for x in metrics_2 if ((x['iteration']%300==299) and (x['iteration']<=3000))], 
        [x['bbox/AP50'] for x in metrics_2 if ((x['iteration']%300==299) and (x['iteration']<=3000))], label='$AP_{50}$ with warmup', color=colors[7], linestyle='-')
    plt.plot(
        [x['iteration'] for x in metrics_1 if ((x['iteration']%300==299) and (x['iteration']<=3000))], 
        [x['keypoints/AP50'] for x in metrics_1 if ((x['iteration']%300==299) and (x['iteration']<=3000))], label='$AP^{kp}_{50}$ no warmup', color=colors[4], linestyle='--')
    plt.plot(
        [x['iteration'] for x in metrics_2 if ((x['iteration']%300==299) and (x['iteration']<=3000))], 
        [x['keypoints/AP50'] for x in metrics_2 if ((x['iteration']%300==299) and (x['iteration']<=3000))], label='$AP^{kp}_{50}$ with warmup', color=colors[7], linestyle='--')
    ...
    plt.grid(b=True, linestyle=':')
    # plt.title('Validation AP50')
    # plt.savefig('/home/porthos/masters_thesis/writing/figures/results/ap50_warmup_comp.pdf')
    plt.show()
...
def draw_train_loss_adam_sgd():
    ...
    plt.legend(['Adam [lr $1e-4$]', 'Adam [lr $1e-3$]', 'SGD [lr $1e-3$]'], 
               loc='upper right')
    plt.xlabel('Iterations')
    plt.ylabel('Total training loss')
    fig_1.tight_layout()
...
def draw_ap50_adam_sgd():
    ...
    plt.legend(['$AP_{50}$ Adam [lr $1e-4$]', '$AP_{50}$ Adam [lr $1e-3$]', '$AP_{50}$ SGD [lr $1e-3$]'], loc='lower right')
    plt.xlabel('Iterations')
    fig_1.tight_layout()
    plt.grid(b=True, linestyle=':')
    ...
    plt.legend(['$AP^{kp}_{50}$ Adam [lr $1e-4$]', '$AP^{kp}_{50}$ Adam [lr $1e-3$]', '$AP^{kp}_{50}$ SGD [lr $1e-3$]'], loc='center right')
    plt.xlabel('Iterations')
    # plt.ylabel('Accuracy')
    fig_2.tight_layout()
    plt.grid(b=True, linestyle=':')
...
def draw_best_model():
    ...
    plt.semilogy(
        [x['iteration'] for x in metrics_1 if x['iteration']<=3000], 
        [x['loss_keypoint'] for x in metrics_1 if x['iteration']<=3000], color=colors[1])
...
def draw_ap50_pretrained():
    ...
    plt.plot(
        [x['iteration'] for x in metrics_2 if ((x['iteration']%300==299) and (x['iteration']<=3000))], 
        [x['bbox/AP50'] for x in metrics_2 if ((x['iteration']%300==299) and (x['iteration']<=3000))], color=colors[0])
    plt.plot(
        [x['iteration'] for x in metrics_1 if ((x['iteration']%300==299) and (x['iteration']<=3000))], 
        [x['keypoints/AP50'] for x in metrics_1 if ((x['iteration']%300==299) and (x['iteration']<=3000))], color=colors[1])
    plt.plot(
        [x['iteration'] for x in metrics_2 if ((x['iteration']%300==299) and (x['iteration']<=3000))], 
        [x['keypoints/AP50'] for x in metrics_2 if ((x['iteration']%300==299) and (x['iteration']<=3000))], color=colors[5])
